...
std::vector<TimedNode*> timedSuccessors_whca(TimedNode* node, map* Map, robot* Robot, std::vector<TimedNode*> reservationTable)
{
    ...
    for (tile* Tile : adj_set)
    {
        ...
        for (int i = 0; i < reservationTable.size(); i++)
        {
            if (reservationTable.at(i)->m_tile == Tile && (reservationTable.at(i)->time == node->time + 1 || (reservationTable.at(i)->time == node->time)))
            {
                occupied = true;
            }
        }
        if (!occupied)
        {
            TimedNode* nd = new TimedNode(Tile, node, calculateHeuristic_Manhattan(Tile, Robot->getTempGoal().at(0), Robot->getTempGoal().at(1)), node->time + 1);
            adj_vec.push_back(nd);
        }
    }
    return adj_vec;
